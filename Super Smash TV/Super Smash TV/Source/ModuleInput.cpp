...
update_status ModuleInput::PreUpdate()
{
	...
	if (pads->x) {
		keys[SDL_SCANCODE_LEFT] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->a) {
		keys[SDL_SCANCODE_DOWN] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->b) {
		keys[SDL_SCANCODE_RIGHT] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->y) {
		keys[SDL_SCANCODE_UP] = KEY_STATE::KEY_REPEAT;
	}
	if (pads->left) {
		keys[SDL_SCANCODE_A] = KEY_STATE::KEY_REPEAT;
	}
